...
use error::CellError;
use libc::{c_int, c_long, c_longlong, size_t, c_double};
use std::error::Error;
use std::iter;
use std::ptr;
use std::string;
...
impl Redis {
    ...
    pub fn reply_double(&self, double: f64) -> Result<(), CellError> {
        handle_status(raw::reply_with_double(self.ctx, double as c_double),
                      "Could not reply with double")
    }
